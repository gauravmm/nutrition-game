...
function assessMeal(total: MacroNutrients): Verdict {
  ...
  if (!energy) return { tone: 'adjust', title: 'Keep building', message: 'Add a few foods, then check your meal.' }
  const protein = total.protein * 4 * 100 / energy
  const fat = total.fat * 9 * 100 / energy
  const carbs = total.carbs * 4 * 100 / energy
  const misses = [protein < 15 || protein > 30, fat < 20 || fat > 35, carbs < 45 || carbs > 65].filter(Boolean).length
  if (!misses) return { tone: 'balanced', title: 'Balanced for this activity', message: 'Your macronutrient mix sits in this activity’s guide ranges. One meal is only one part of an overall diet.' }
  if (misses === 1) return { tone: 'nearly', title: 'Close — try one small change', message: 'Try varying the mix of foods on your plate. One meal is only one part of an overall diet.' }
  return { tone: 'adjust', title: 'Try adjusting your meal', message: 'Try changing a portion or adding a different kind of food, then check again. One meal is only one part of an overall diet.' }
}
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="workspace">
        ...
        <aside className={`result-panel ${submitted ? 'is-revealed' : ''}`} aria-labelledby="results-title"><h2 id="results-title">Your meal</h2>{!submitted ? <div className="hidden-results"><span className="secret-mark" aria-hidden="true">?</span><h3>{lastChecked ? 'Your meal changed' : 'Keep building'}</h3><p>{lastChecked ? 'Check again to reveal updated nutrition.' : 'When you’re ready, check your meal to reveal its energy and macronutrients.'}</p></div> : <div className="results-content"><div className="calorie-total"><span>Total energy</span><strong>{rounded(total.calories)}</strong><em>kcal</em></div><MacroBar total={total} /><div className={`verdict ${assessMeal(total).tone}`}><span aria-hidden="true">{assessMeal(total).tone === 'balanced' ? '✓' : '↗'}</span><div><h3>{assessMeal(total).title}</h3><p>{assessMeal(total).message}</p></div></div><details className="contributions"><summary>Food contributions and sources</summary><ul>{plate.map((item) => { const itemTotal = multiply(item.food.macros, item.portion.multiplier); return <li key={item.instanceId}><span>{item.food.name} · {item.portion.label}</span><strong>{rounded(itemTotal.calories)} kcal</strong><small>{rounded(itemTotal.carbs)} g carbs · {rounded(itemTotal.protein)} g protein · {rounded(itemTotal.fat)} g fat</small><a href={item.food.source.url} target="_blank" rel="noreferrer">{item.food.source.name} ↗<span className="sr-only"> (opens in a new tab)</span></a></li> })}</ul><p>Values can vary by recipe and preparation.</p></details>{previous && <p className="previous-check">Previous check: {rounded(previous.calories)} kcal</p>}<button className="retry-button" onClick={() => setSubmitted(false)} type="button">Adjust and check again</button></div>}<button className="check-button" onClick={checkMeal} disabled={!plate.length} type="button">Check my meal <span aria-hidden="true">→</span></button></aside>
      </div>
